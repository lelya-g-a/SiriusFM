...
namespace SiriusFM
{
        ...
        double              a_BFactor)    // #StdDevs for Upper Boundary
    {
        ...
        {
            throw std::invalid_argument
                ("Option has already expired, or too close");
        }
        ...
        m_i0 = int(round(a_S0 / h)); // 1 idx corresp to S0
        h = a_S0 / double(m_i0);
        if (!std::isfinite(h))
        {
        ...
        // At low Bound (a=0), we always have a constant boundary cond, 
        // continuous with payoff, or 0 in Fwd run:
        double fa = IsFwd ? 0 : payOff[0];

        // At the Upper Bound? we use a const boundary cond if it is 0,
        // otherwise we fix the df/dS (a Neumann-type cond -- Bwd only):
                     ...
                     j += (IsFwd ? 1 : -1))
        {
            ...
            if (a_option -> IsAmerican())
            {
                ...
                for (int i = 0; i < m_N; ++i)
                {
                    // Intrinsic value of the option is the payoff evaluated
                    // under the curr underlying px Si:
    ...
    // "GEtPxDeltaGamma0": Assume S0 is on the Grid (Bwd):
    template <typename Diffusion1D,
              typename AProvider,
              typename BProvider,
              typename AssetClassA,
